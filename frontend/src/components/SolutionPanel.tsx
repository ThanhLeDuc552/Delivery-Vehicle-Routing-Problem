...
import { Truck, Route as RouteIcon, AlertCircle, XCircle, HelpCircle, CheckCircle, Clock, AlertTriangle } from 'lucide-react';
...
export function SolutionPanel({ solution, solutionStatus }: SolutionPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <>
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div className="p-3 border rounded-md bg-card">
              ...
              <div>{solution.summary.numberOfRoutes}</div>
            </div>
            <div className="p-3 border rounded-md bg-card">
              <div className="flex items-center gap-2 text-muted-foreground mb-1">
              ...
              <div>{solution.summary.totalDistance.toFixed(1)}</div>
            </div>
            <div className="p-3 border rounded-md bg-card">
              <div className="flex items-center gap-2 text-muted-foreground mb-1">
              ...
              <div>{(solution.summary.deliveryRate * 100).toFixed(0)}%</div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="p-3 border rounded-md bg-card">
              <div className="text-xs text-muted-foreground mb-1">Items Delivered</div>
              <div>{solution.summary.totalItemsDelivered} / {solution.summary.totalItemsRequested}</div>
            </div>
            <div className="p-3 border rounded-md bg-card">
              <div className="text-xs text-muted-foreground mb-1">Unserved</div>
              <div>{solution.summary.unservedCustomers} customers</div>
            </div>
          </div>
        </>
      ) : (
      ...
      {/* Metadata */}
      {(solution.meta || solution.solveTimeMs) && (
        <div className="mb-4 p-3 border rounded-md bg-card/50">
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2 text-muted-foreground">
              <Clock className="h-3 w-3" />
              <span>{solution.meta?.solver || solution.configName || 'Solver'}</span>
            </div>
            <span className="text-muted-foreground">
              {solution.solveTimeMs || solution.meta?.solve_time_ms}ms
            </span>
          </div>
        </div>
      )}
      ...
      {solution.unservedCustomers && solution.unservedCustomers.length > 0 && (
        <Alert className="mb-4">
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>
            {solution.unservedCustomers.length} customer(s) could not be served: {solution.unservedCustomers.map(c => c.name).join(', ')}
          </AlertDescription>
        </Alert>
      )}
