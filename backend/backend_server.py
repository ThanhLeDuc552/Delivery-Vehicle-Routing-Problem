...
def submit_solution():
    ...
    if action == 'response':
        ...
        try:
            ...
            # Validate required fields - new format with depot, vehicles, customers
            if 'depot' not in cvrp_data:
                return jsonify({'error': 'Missing required field: depot'}), 400
            if 'vehicles' not in cvrp_data or not isinstance(cvrp_data['vehicles'], list):
                return jsonify({'error': 'Missing or invalid field: vehicles (must be array)'}), 400
            ...
            if 'name' not in depot or 'x' not in depot or 'y' not in depot:
                return jsonify({'error': 'Depot must have: name, x, y'}), 400
            
            # Validate vehicles structure
            for i, vehicle in enumerate(cvrp_data['vehicles']):
                if 'name' not in vehicle or 'capacity' not in vehicle or 'maxDistance' not in vehicle:
                    return jsonify({'error': f'Vehicle {i} must have: name, capacity, maxDistance'}), 400
            ...
            print(f"Received CVRP request from frontend: {len(cvrp_data['customers'])} customers, {len(cvrp_data['vehicles'])} vehicles")
